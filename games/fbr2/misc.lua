...
Players.PlayerAdded:Connect(function(player)
    player.CharacterAdded:Connect(function()
        wait(0.5)
        if _G.UNDEITEDHUB_WINDOW_VISIBLE then setupPlayer(player.Name) end
    end)
    if Workspace:FindFirstChild(player.Name) and _G.UNDEITEDHUB_WINDOW_VISIBLE then
        setupPlayer(player.Name)
    end
end)

Workspace.ChildAdded:Connect(function(child)
    if child:IsA("Model") and Players:FindFirstChild(child.Name) and _G.UNDEITEDHUB_WINDOW_VISIBLE then
        wait(0.5)
