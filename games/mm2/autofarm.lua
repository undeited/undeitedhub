...
local function GetCurrentMap()
    for _, child in ipairs(workspace:GetChildren()) do
        if child:IsA("Model") and child:FindFirstChild("CoinContainer") then return child end
    end
    return nil
end
    ...
    if #coins == 0 then
        for _, obj in ipairs(workspace:GetDescendants()) do
            ...
            elseif obj:IsA("Model") and obj:FindFirstChild("TouchInterest") then
                local ok = false
                for _, d in ipairs(obj:GetDescendants()) do if d:IsA("BasePart") then ok = true break end end
...
local function GetCoinPosition(coin)
    ...
    if coin.PrimaryPart then return coin.PrimaryPart.Position end
    for _, part in ipairs(coin:GetDescendants()) do if part:IsA("BasePart") then return part.Position end end
    return nil
end
...
local function TeleportToLobby()
    ...
    local spawnPoints = {}
    for _, child in ipairs(spawns:GetChildren()) do if child:IsA("BasePart") then table.insert(spawnPoints, child) end end
...
local function onCoinCollected(bagName, newCount, maxCount, data)
    ...
    if newCount >= maxCount then
        bagFull = true
        if collectRunning then SafeNotify({ Title = "Autofarm", Content = "Bag full! Pausing...", Duration = 2 }) end
        if autoTeleportToLobbyEnabled and not IsInLobby() then
            ...
            if now - lastTeleportTime >= TELEPORT_COOLDOWN then
                ...
                if success then SafeNotify({ Title = "Autofarm", Content = "Teleported to lobby", Duration = 2 })
                else SafeNotify({ Title = "Autofarm", Content = "Failed to teleport to lobby", Duration = 2 }) end
            end
        end
        if killAfterFullEnabled and not killAfterFullCooldown then
            ...
            task.spawn(function()
                if IsLocalPlayerMurderer() then
                    pcall(function() if undeitedhub.KillAll then undeitedhub.KillAll() end end)
                    task.wait(1)
                    bagFull = false
                    refreshNeeded = true
                    killAfterFullCooldown = false
                    if collectRunning then SafeNotify({ Title = "Autofarm", Content = "Bag reset after kill all", Duration = 2 }) end
                else
                    bagFull = false
                    refreshNeeded = true
                    killAfterFullCooldown = false
                    if collectRunning then SafeNotify({ Title = "Autofarm", Content = "Bag full but not murderer, continuing", Duration = 2 }) end
                end
            end)
        end
    else
...
local function ScanAndDisableKillBricks()
    for _, obj in ipairs(workspace:GetDescendants()) do
        if obj:IsA("BasePart") and (obj.Name == "KillBrick" or string.find(obj.Name, "KillBrick")) then DisableKillBrick(obj) end
    end
end
local function SetupKillBrickDisable()
    for _, conn in ipairs(killBrickConnections) do conn:Disconnect() end
    killBrickConnections = {}
    if killBrickAddedConnection then killBrickAddedConnection:Disconnect(); killBrickAddedConnection = nil end
    if not killBrickDisableActive then return end
    ScanAndDisableKillBricks()
    killBrickAddedConnection = workspace.DescendantAdded:Connect(function(obj)
        if killBrickDisableActive and obj:IsA("BasePart") and (obj.Name == "KillBrick" or string.find(obj.Name, "KillBrick")) then DisableKillBrick(obj) end
    end)
    table.insert(killBrickConnections, killBrickAddedConnection)
    local function onCharacterAdded(char)
        local function checkParts()
            for _, part in ipairs(char:GetDescendants()) do
                if part:IsA("BasePart") and (part.Name == "KillBrick" or string.find(part.Name, "KillBrick")) then DisableKillBrick(part) end
            end
        end
        checkParts()
    ...
    for _, player in ipairs(game.Players:GetPlayers()) do
        ...
        local addedConn = player.CharacterAdded:Connect(function(char) task.wait(0.1); onCharacterAdded(char) end)
...
local function StartMonitoring()
    ...
    monitorConnection = RunService.Heartbeat:Connect(function()
        if collectRunning and _G.UNDEITEDHUB_WINDOW_VISIBLE then pcall(MonitorCoin) end
    end)
end
...
local function CollectCoins()
    if GetCurrentMap() == nil or IsInLobby() or not IsRoundActive() or not IsPlayerAlive() or waitingForRoundStart then return end
    local coinCount = GetCoinCount()
    if coinCount >= 40 then bagFull = true end
    if bagFull then task.wait(0.5); return end
    local coinServers = GetAllActiveCoinServers()
    if refreshNeeded then
        coinServers = GetAllActiveCoinServers()
        refreshNeeded = false
    end
    if #coinServers == 0 then task.wait(0.5); return end
    local localPlayer = game.Players.LocalPlayer
    if not localPlayer then return end
    local char = localPlayer.Character
    ...
    -- Use optimal order (nearest neighbor) instead of just nearest
    local positions = {}
    for _, coin in ipairs(coinServers) do
        local pos = GetCoinPosition(coin)
        if pos then table.insert(positions, {coin = coin, pos = pos}) end
    end
    if #positions == 0 then return end
    local ordered = Math.optimalOrder(positions, root.Position)
    local targetCoin = ordered[1].coin
    if currentTarget == targetCoin and targetCoin:FindFirstChild("TouchInterest") then return end
    if currentTween then pcall(currentTween.Cancel, currentTween); currentTween = nil end
    if not targetCoin:FindFirstChild("TouchInterest") and not targetCoin:IsA("Part") then return end
    currentTarget = targetCoin
    local coinPos = GetCoinPosition(targetCoin)
    if not coinPos then currentTarget = nil; return end
    ...
    if camera then camera.CameraSubject = nil end
    ...
    _G.UNDEITEDHUB_AUTOFARM_MOVING = false
    if camera and originalSubject then camera.CameraSubject = originalSubject end
    if not success then
    ...
    if newCount >= 40 then bagFull = true end
    ...
    local function onCharacterAdded(newChar)
        ...
        if humanoid then
            healthChangedConnection = humanoid.HealthChanged:Connect(function(health)
                if health <= 0 then cancelTweenAndTarget() end
            end)
        end
        characterRemovingConnection = newChar.AncestryChanged:Connect(function()
            if not newChar.Parent then cancelTweenAndTarget() end
        end)
    end
    ...
    collectTask = task.spawn(function()
        while collectRunning do
            while collectRunning and (IsInLobby() or not IsRoundActive() or not IsPlayerAlive() or waitingForRoundStart or not _G.UNDEITEDHUB_WINDOW_VISIBLE) do
                if collectRunning and not IsInLobby() and IsRoundActive() and IsPlayerAlive() and _G.UNDEITEDHUB_WINDOW_VISIBLE then waitingForRoundStart = false end
                task.wait(0.5)
            end
            ...
            while collectRunning and not IsInLobby() and IsRoundActive() and IsPlayerAlive() and not waitingForRoundStart and _G.UNDEITEDHUB_WINDOW_VISIBLE do
                pcall(CollectCoins)
                if not currentTarget then task.wait(0.2 + math.random() * 0.3)
...
AutofarmTab:Toggle({ Title = "Auto Collect Coins", Value = autoCollectEnabled, Callback = function(state) if state then StartAutoCollect() else StopAutoCollect() end end })
AutofarmTab:Toggle({ Title = "Auto Kill All when Bag Full", Value = killAfterFullEnabled, Callback = function(state) killAfterFullEnabled = state; undeitedhub.Toggles.killAfterFullEnabled = state; if undeitedhub.SaveSettings then undeitedhub.SaveSettings() end; SafeNotify({ Title = "Auto Kill All when Bag Full", Content = state and "Enabled" or "Disabled", Duration = 2 }) end })
AutofarmTab:Toggle({ Title = "Auto Teleport to Lobby when Bag Full", Value = autoTeleportToLobbyEnabled, Callback = function(state) autoTeleportToLobbyEnabled = state; undeitedhub.Toggles.autoTeleportToLobbyEnabled = state; if undeitedhub.SaveSettings then undeitedhub.SaveSettings() end; SafeNotify({ Title = "Auto Teleport to Lobby", Content = state and "Enabled" or "Disabled", Duration = 2 }); if state then lastTeleportTime = tick() end end })
