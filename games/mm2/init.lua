...
local function LoadString(script, chunkName)
    if type(loadstring) == "function" then return loadstring(script, chunkName) end
    if type(load) == "function" then return load(script, chunkName) end
...
local function CheckExecutor()
    ...
    if #missing > 0 then
        pcall(function() game:GetService("StarterGui"):SetCore("SendNotification", { Title = "Executor Incompatible", Text = "Missing executor support: " .. table.concat(missing, ", "), Duration = 5 }) end)
        return false
    end
    return true
end
...
local function ResolveThemeName(themeName)
    ...
    if type(themeName) ~= "string" or themeName == "" then return available[1] or "Default" end
    if themeName == "Undelted" then return "Default" end
    for _, name in ipairs(available) do if name == themeName then return name end end
...
local function LoadSettings()
    pcall(function()
        if isfile and isfile(undeitedhub.SettingsFile) then
            ...
            if data then
                if data.toggles then for k, v in pairs(data.toggles) do undeitedhub.Toggles[k] = v end end
                if data.theme then undeitedhub.CurrentTheme = ResolveThemeName(data.theme) end
                if data.toggleKey then undeitedhub.ToggleKey = data.toggleKey end
        ...
        if _G.UNDEITEDHUB_STORAGE and _G.UNDEITEDHUB_STORAGE[game.PlaceId] then
            ...
            if stored.toggles then for k, v in pairs(stored.toggles) do undeitedhub.Toggles[k] = v end end
            if stored.theme then undeitedhub.CurrentTheme = ResolveThemeName(stored.theme) end
            if stored.toggleKey then undeitedhub.ToggleKey = stored.toggleKey end
...
local function SaveSettings()
    pcall(function()
        local data = { toggles = undeitedhub.Toggles, theme = ResolveThemeName(undeitedhub.CurrentTheme or "Default"), toggleKey = undeitedhub.ToggleKey or config.toggleKey or "K", walkSpeed = config.walkSpeed, jumpPower = config.jumpPower }
...
local Window = WindUI:CreateWindow({ Title = "Undeited Hub", Author = "by undeited", Folder = "undeitedhub/" .. GAME_FOLDER, Size = UDim2.fromOffset(580, 460), MinSize = Vector2.new(560, 350), MaxSize = Vector2.new(850, 560), Transparent = true, Theme = themeToApply, Resizable = true, SideBarWidth = 200, HideSearchBar = true, ScrollBarEnabled = false })
...
if frame then
    frame:GetPropertyChangedSignal("Visible"):Connect(function() _G.UNDEITEDHUB_WINDOW_VISIBLE = frame.Visible end)
    frame.AncestryChanged:Connect(function() if not frame.Parent then if undeitedhub.DisableAll then undeitedhub.DisableAll() end end end)
end
local function SafeLoad(name)
    local success, err = pcall(LoadScript, name)
    if not success then
        WindUI:Notify({ Title = "Error: " .. name, Content = tostring(err):sub(1, 200), Duration = 6 })
    end
end
...
if _G.UNDEITEDHUB_STATES then
    for key, value in pairs(_G.UNDEITEDHUB_STATES) do undeitedhub.Toggles[key] = value end
    _G.UNDEITEDHUB_STATES = nil
end
if undeitedhub.RestoreStates then undeitedhub.RestoreStates() end
SaveSettings()