...
local function IsInLobby()
    ...
    if roundTimer then
        ...
        if time ~= nil and time > 0 then return false end
    end
...
local function IsRoundActive()
    if not roundTimer then return not IsInLobby() end
    local time = roundTimer:GetAttribute("Time")
    if time == nil then return not IsInLobby() end
    return time > 0
end
local function normalizeRole(role)
    if type(role) == "table" then role = role.Role or role.Name or tostring(role) end
    if type(role) ~= "string" then return "Innocent" end
    local lower = string.lower(role)
    if lower == "murderer" or lower == "killer" then return "Murderer"
    elseif lower == "sheriff" or lower == "hero" or lower == "sheriff (hero)" then return "Sheriff"
    else return "Innocent" end
end
local function playerHasToolPattern(player, pattern)
    if not player then return false end
    local backpack = player:FindFirstChild("Backpack")
    if backpack then for _, tool in ipairs(backpack:GetChildren()) do if tool:IsA("Tool") and string.lower(tool.Name):find(pattern) then return true end end end
    local character = player.Character
    if character then for _, tool in ipairs(character:GetChildren()) do if tool:IsA("Tool") and string.lower(tool.Name):find(pattern) then return true end end end
    return false
end
local function detectRoleFromTools(player)
    if playerHasToolPattern(player, "knife") or playerHasToolPattern(player, "murderer") then return "Murderer"
    elseif playerHasToolPattern(player, "gun") or playerHasToolPattern(player, "sheriff") or playerHasToolPattern(player, "revolver") then return "Sheriff"
    else return "Innocent" end
end
local function GetPlayerRole(player)
    if not player then return "Innocent" end
    if undeitedhub.playerRoles and undeitedhub.playerRoles[player] then return normalizeRole(undeitedhub.playerRoles[player]) end
    ...
    if char then
        ...
        if roleVal and roleVal:IsA("StringValue") then return normalizeRole(roleVal.Value) end
    end
...
local function GetPlayerRoleColor(player)
    ...
    if undeitedhub.GetCurrentMurderer and undeitedhub.GetCurrentMurderer() == player then return config.colors.murderer or Color3.fromRGB(255, 0, 0) end
    if undeitedhub.GetCurrentSheriff and undeitedhub.GetCurrentSheriff() == player then return config.colors.sheriff or Color3.fromRGB(0, 100, 255) end
    local role = GetPlayerRole(player)
    if role == "Murderer" then return config.colors.murderer or Color3.fromRGB(255, 0, 0)
    elseif role == "Sheriff" then return config.colors.sheriff or Color3.fromRGB(0, 100, 255)
    else return config.colors.innocent or Color3.fromRGB(0, 255, 0) end
end
local function ClearESP()
    for _, highlight in pairs(highlightInstances) do if highlight and highlight.Parent then pcall(highlight.Destroy, highlight) end end
    highlightInstances = {}
end
local function UpdateESP()
    if not espEnabled then ClearESP(); return end
    if IsInLobby() or not IsRoundActive() then ClearESP(); return end
    local localPlayer = game.Players.LocalPlayer
    ...
    for player, _ in pairs(playersToHighlight) do
        ...
        if roleColor then
            ...
            if highlight then
                pcall(function() highlight.FillColor = roleColor; highlight.OutlineColor = roleColor end)
            else
                pcall(function()
...
local function ClearGunHighlights()
    for _, highlight in pairs(gunHighlightInstances) do if highlight and highlight.Parent then pcall(highlight.Destroy, highlight) end end
    gunHighlightInstances = {}
end
local function UpdateGunHighlights()
    if not gunHighlightEnabled then ClearGunHighlights(); return end
    if IsInLobby() or not IsRoundActive() then ClearGunHighlights(); return end
    local gunDrops = {}
    for _, obj in pairs(workspace:GetDescendants()) do if obj.Name == "GunDrop" then table.insert(gunDrops, obj) end end
...
local function ClearCoinHighlights()
    for part, highlight in pairs(coinHighlightMap) do if highlight and highlight.Parent then pcall(highlight.Destroy, highlight) end end
    coinHighlightMap = {}
end
local function UpdateCoinHighlights()
    if not coinHighlightEnabled then ClearCoinHighlights(); return end
    if IsInLobby() or not IsRoundActive() then ClearCoinHighlights(); return end
    local currentParts = GetAllCoinParts()
    local currentSet = {}
    for _, part in ipairs(currentParts) do currentSet[part] = true end
...
if gameplay then
    ...
    if playerDataChanged and playerDataChanged:IsA("RemoteEvent") then
        playerDataChanged.OnClientEvent:Connect(function(data) pcall(function() updateRolesFromData(data) end) end)
    end
    local roleData = gameplay:FindFirstChild("RoleData")
    if roleData and roleData:IsA("RemoteEvent") then
        roleData.OnClientEvent:Connect(function(data) pcall(function() updateRolesFromData(data) end) end)
    end
end
local function updateRolesFromData(data)
    ...
    for playerName, role in pairs(data) do
        ...
        if player then
            ...
            if type(normalized) == "string" then player:SetAttribute("Role", normalized) end
        end
    end
    UpdateESP()
end
workspace.DescendantAdded:Connect(function(obj) if obj.Name == "GunDrop" and gunHighlightEnabled and not IsInLobby() and IsRoundActive() then UpdateGunHighlights() end end)
workspace.DescendantRemoving:Connect(function(obj) if obj.Name == "GunDrop" and not IsInLobby() and IsRoundActive() then pcall(UpdateESP) end end)
local function forceRoleScan()
    ...
    if changed then UpdateESP() end
end
undeitedhub.ForceRoleScan = forceRoleScan
task.spawn(function()
    while true do task.wait(0.5); pcall(forceRoleScan) end
end)
local function setupToolListener(player)
    ...
    local function onCharacterAdded(char)
        ...
        local function onDescendantAdded(desc)
            if desc:IsA("Tool") and string.lower(desc.Name):find("gun") then pcall(function() UpdateESP(); forceRoleScan() end) end
        end
        ...
        for _, child in ipairs(char:GetChildren()) do
            if child:IsA("Tool") and string.lower(child.Name):find("gun") then pcall(function() UpdateESP(); forceRoleScan() end); break end
        end
    end
    if player.Character then onCharacterAdded(player.Character) end
    player.CharacterAdded:Connect(onCharacterAdded)
end
for _, player in ipairs(game.Players:GetPlayers()) do setupToolListener(player) end
...
if roundTimer then roundTimer:GetAttributeChangedSignal("Time"):Connect(refreshRoundState) end
game:GetService("RunService").Heartbeat:Connect(function()
    pcall(refreshRoundState)
    if coinHighlightEnabled and not IsInLobby() and IsRoundActive() then
        local now = tick()
        if now - coinHighlightLastUpdate > 2 then coinHighlightLastUpdate = now; pcall(UpdateCoinHighlights) end
    end
end)
game.Players.PlayerAdded:Connect(function(player)
    player.CharacterAdded:Connect(function() task.wait(0.5); pcall(forceRoleScan) end)
    player:GetAttributeChangedSignal("Role"):Connect(function() pcall(UpdateESP) end)
end)
game.Players.PlayerRemoving:Connect(function(player)
    if highlightInstances[player] then pcall(function() highlightInstances[player]:Destroy(); highlightInstances[player] = nil end) end
    undeitedhub.playerRoles[player] = nil
end)
VisualTab:Toggle({ Title = "Player Highlight", Value = espEnabled, Callback = function(state)
    pcall(function()
        espEnabled = state
...
VisualTab:Toggle({ Title = "Gun Highlight", Value = gunHighlightEnabled, Callback = function(state)
    pcall(function()
        gunHighlightEnabled = state
        undeitedhub.Toggles.gunHighlightEnabled = state
        if undeitedhub.SaveSettings then undeitedhub.SaveSettings() end
        SafeNotify({ Title = "Gun Highlight", Content = gunHighlightEnabled and "Enabled" or "Disabled", Duration = 2 })
        if not gunHighlightEnabled then ClearGunHighlights() else UpdateGunHighlights() end
    end)
end })
VisualTab:Toggle({ Title = "Coin Highlight", Value = coinHighlightEnabled, Callback = function(state)
    pcall(function()
        coinHighlightEnabled = state
        undeitedhub.Toggles.coinHighlightEnabled = state
        if undeitedhub.SaveSettings then undeitedhub.SaveSettings() end
        SafeNotify({ Title = "Coin Highlight", Content = coinHighlightEnabled and "Enabled" or "Disabled", Duration = 2 })
        if not coinHighlightEnabled then ClearCoinHighlights() else UpdateCoinHighlights() end
    end)
end })
...
if espEnabled then task.spawn(function() task.wait(0.5); refreshRoundState(); UpdateESP() end) end