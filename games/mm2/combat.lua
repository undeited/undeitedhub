...
game:GetService("RunService").Heartbeat:Connect(function()
    if autoKillAllEnabled and _G.BANDITHUB_WINDOW_VISIBLE then
        local now = tick()
...
local function ShootMurdererOnce()
    if not _G.BANDITHUB_WINDOW_VISIBLE then return false end
    local localPlayer = game.Players.LocalPlayer
    if not localPlayer then return false end
    if not IsRoundActive() or not IsPlayerAlive() or IsInLobby() then
...
game:GetService("RunService").Heartbeat:Connect(function()
    if autoShootEnabled and _G.BANDITHUB_WINDOW_VISIBLE then
        local now = tick()
...
local autoGrabGunEnabled = bandithub.Toggles.autoGrabGunEnabled or false
local gunDropAddedConnection = nil

local function AttemptAutoGrab()
    if not autoGrabGunEnabled or not _G.BANDITHUB_WINDOW_VISIBLE then return end
...
local function ToggleAutoGrab(state)
    ...
    bandithub.Toggles.autoGrabGunEnabled = state
    if bandithub.SaveSettings then bandithub.SaveSettings() end
    ...
    if state then
        ...
        gunDropAddedConnection = workspace.DescendantAdded:Connect(function(obj)
            if autoGrabGunEnabled and obj.Name == "GunDrop" and _G.BANDITHUB_WINDOW_VISIBLE then
                pcall(AttemptAutoGrab)
            end
        end)
...
bandithub.DisableAll = function()
    ...
    bandithub.Toggles.autoGrabGunEnabled = false
    if bandithub.SaveSettings then bandithub.SaveSettings() end
    if gunDropAddedConnection then
        gunDropAddedConnection:Disconnect()
        gunDropAddedConnection = nil
