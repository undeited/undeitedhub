...
local BLOBMAN_VOID_THRESHOLD = 50
        ...
        while true do
            if _G.UNDEITEDHUB_WINDOW_VISIBLE then
                ...
                if #blobmen == 0 then
                    ...
                    local root = blobman:FindFirstChild("HumanoidRootPart") or blobman.PrimaryPart or blobman:FindFirstChild("VehicleSeat")
                    if root then
                        local deathHeight = Workspace.FallenPartsDestroyHeight
                        if not deathHeight or deathHeight == math.huge or deathHeight == -math.huge then
                            deathHeight = -500
                        end
                        if root.Position.Y <= deathHeight + BLOBMAN_VOID_THRESHOLD then
                            deleteToy(blobman)
...
local function getPlayerFromDropdownValue(value)
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LocalPlayer then
            ...
            if value == player.DisplayName .. " (@)" .. player.Name .. ")" then
                return player
            end
        end
    end
