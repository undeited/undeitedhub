...
local function playGrabAnimation(blobman, side)
    ...
    if side == "left" then
        animName = "LeftGrabAnimation"
    else
        animName = "RightGrabAnimation"
    end
...
local function dropHeldTarget(blobman, side)
    ...
    if side == "left" then
        target = leftHeldTarget
    else
        target = rightHeldTarget
    end
    ...
    local leftWeld = leftDetector
        and leftDetector:FindFirstChild("LeftWeld")

    local rightWeld = rightDetector
        and rightDetector:FindFirstChild("RightWeld")

    local ownerScript = blobman:FindFirstChild(
        "BlobmanSeatAndOwnerScript"
    )

    local creatureDrop = ownerScript
...
local function grabPlayer(blobman, target, hand)
    ...
    local creatureGrab = ownerScript
...
local function bringSelectedPlayer()
    if selectedBringPlayer == nil or selectedBringPlayer == "" then
        return
    end
...
local oldDisable = undeitedhub.DisableAll or function()
end
