...
TrollTab:Button({
    ...
    Callback = function()
        ...
        if not spawnRemote then
            SafeNotify({ Title = "Burn All", Content = "SpawnRemote not found", Duration = 2 })
            return
        end
        ...
        for _, player in ipairs(Players:GetPlayers()) do
            ...
            if success then spawned = spawned + 1 end
            task.wait(0.05)
        end

        SafeNotify({ Title = "Burn All", Content = "Spawned " .. spawned .. " campfires", Duration = 2 })
    end
})
